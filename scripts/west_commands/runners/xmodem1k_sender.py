...
class Xmodem1KSender(ZephyrBinaryRunner):
    def __init__(self, cfg: RunnerConfig, dev_id: str | None, file: str | None, flash_header: FlashHeader | None):
        super().__init__(cfg)
        ...
        if file is None:
            raise RuntimeError(
                "XMODEM1k binary runner is missing path to file that should be sent"
            )
    ...
    def do_add_parser(cls, parser: argparse.ArgumentParser) -> None:
        ...
        parser.add_argument("--no-header", action='store_true', help="Don't include the flash header and only send the plain binary file via the XMODEM1k protocol")

        parser.add_argument("--flash-offset", type='int_auto_base', required=False)
        parser.add_argument("--no-verify", action='store_true', help="Don't do flash verifications directly after writing")
